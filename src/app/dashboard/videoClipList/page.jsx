...
const Page = () => {
  ...
  const createSampleVideoClipList = async () => {
    ...
    try {
      ...
      router.push("/dashboard/videoClipList/" + response.data._id);
      toast.success("Sample Video Lesson Created Successfully!", {
        position: "top-center",
      });
    } catch (error) {
      if (error?.response?.status == 401) {
  ...
  const deleteVideoClipList = async () => {
    dispatchUserData({ type: "checkLogin" });

    const config = {
      method: "delete",
      url: "/api/videoClipList/" + deleteId,
      headers: {
        Authorization: `Bearer ${getToken("token")}`,
      },
    };
    ...
    <div className="container mx-auto py-4 px-4 md:px-0">
      <div>
        <div className="w-full flex justify-end pb-3">
          <button
            type="button"
            className="px-3 py-2 text-xs font-medium text-center inline-flex items-center text-white rounded-lg focus:ring-4 focus:outline-none bg-green-600 hover:bg-green-700 focus:ring-green-800"
            onClick={() => createSampleVideoClipList()}
          ...
          >
            ...
            {createLoading ? "Creating..." : "New Video Clip"}
          </button>
        </div>
        <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
          <table className="w-full text-sm text-left text-gray-400">
            ...
            {!listLoading && (
              <tbody>
                {videoClipListList?.videoClipLists?.map((item, index) => (
                  <tr
                    className="border-b  bg-gray-800 border-gray-700 hover:bg-gray-600"
                    key={index}
                  >
                    <td className="px-6 py-4">{item.videoTitle}</td>
                    <td className="px-6 py-4">{item.videoDetailsText}</td>
                    <td className="px-6 py-4">{item.videoKeywords}</td>
                    ...
                    <td className="px-6 py-4">
                      <Link
                        target="_blank"
                        className="underline text-blue-600"
                        href={`https://youtu.be/${item.videoLink}`}
                      >
                        https://youtu.be/{item.videoLink}
                      </Link>{" "}
                    </td>
                    <td className="px-6 py-4">
                      <Link
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="inline-flex space-x-1 items-center text-base font-semibold text-white">
                        <button
                          ...
                          onClick={() =>
                            router.push("/dashboard/videoClipList/" + item._id)
                          }
                        >
                          <svg
                            className="w-3 h-3 mr-1 text-white"
